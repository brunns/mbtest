...
def test_executable_not_mb():
    with pytest.raises(MountebankTimeoutError, match="Mountebank failed to start"):
        ExecutingMountebankServer(executable="ls", port=2526)
...
def test_exception_running_multiple_servers_on_same_port():
    ...
    try:
        ...
        with pytest.raises(MountebankPortInUseException, match="Already running on port 2526"):
...
def test_allow_multiple_servers_on_different_ports():
    ...
    try:
        ...
        with server1(imposter1), server2(imposter2):
            response1 = httpx.get(f"{imposter1.url}/test")
            response2 = httpx.get(f"{imposter2.url}/test")
...
def test_query_all_imposters(mock_server):
    ...
    with mock_server([imposter1, imposter2]) as server:
        ...
        assert_that(
            actual, contains_inanyorder(has_identical_properties_to(imposter1), has_identical_properties_to(imposter2))
        )
...
def test_remove_and_replace_impostor_from_running_server(mock_server):
    ...
    with mock_server(
        ...
        responses = [httpx.get(f"{initial[i].url}/test") for i in range(3)]
        assert_that(
            responses,
            contains_inanyorder(
        ...
        egg_impostor = next(i for i in initial if i.name == "egg")
